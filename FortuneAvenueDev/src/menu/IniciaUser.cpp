...
std::string wideToString(const std::wstring& wideStr) {
	...
	return strTo;
#else
	// En sistemas no Windows, podemos usar std::wstring_convert o m�todos alternativos
...
std::string openFileDialog() {
	...
	// Crear un cuadro de di�logo de selecci�n de archivo
	...
	// Establecer el filtro para mostrar solo im�genes
			...
			if (event.type == sf::Event::MouseMoved) {
				if (dragging) {
					...
					if (resizingCorner == 0 || resizingCorner == 1) {  // Esquina superior izquierda
						...
						newPos.y -= mousePos.x - (selectionBox.getPosition().x + selectionBox.getSize().x); //  Ajustar posici�n en Y para que la reducci�n sea hacia abajo
					}
