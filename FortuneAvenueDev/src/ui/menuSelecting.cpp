...
void menuSelecting::setPosition(sf::Vector2f Position) {
	...
	if (Menu.empty()) {
		std::cerr << "Error: El vector Menu est� vac�o." << std::endl;
		return;  // Salir si el vector est� vac�o
	}
	Selection = Menu[SelecIdio];
	Selection.setPosition(position);
	...
	for (auto& tex : Menu) {
		if (tex.getFont() == nullptr) {
			...
			continue;  // Continuar si la fuente no est� asignada
		}
			...
			for (size_t i = 0; i < Menu.size(); ++i) {
				...
				if (hovered) {
					...
					if (event.type == sf::Event::MouseButtonPressed && event.mouseButton.button == sf::Mouse::Left) {
						...
						open = false;  // Se clicke�
					}
				}
				else {
					Menu[i].setCharacterSize(23); // Tama�o normal
				}
			}
		}
	}
