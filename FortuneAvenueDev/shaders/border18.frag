...
void main(){
    ...
    for (float x=-thickness*4.0;x<=thickness*4.0;x+=1.0) for (float y=-thickness*4.0;y<=thickness*4.0;y+=1.0) outer = max(outer, tex(uv+vec2(x,y)/texSize).a);
    if (a>0.0) gl_FragColor = tex(uv);
    else if (inner>0.0) gl_FragColor = outlineColor;
    else if (outer>0.0) gl_FragColor = vec4(outlineColor.rgb, 0.5);
    else discard;
}
