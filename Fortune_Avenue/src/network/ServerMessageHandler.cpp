...
//	std::cout << "Paquete recibido de tamaño " << preprocces->dataLength << " bytes.\n";
	...
	//	playerGameNew.textureAvatarPLayer.loadFromFile(playerInfoNew.image);
		...
		PlantillaMensajeR.NombreChat.setString(playersGame[num].NamePlayer.getString());
		...
		if (In == 1) {
			...
			PlantillaMensajeR.NombreChat.setPosition(970, PlantillaMensajeR.ContenidoEnviado.getPosition().y - 10);
		...
		for (int i = 0; i < Mensajes.size() - 1; i++)
		{
			...
			Mensajes[i].NombreChat.setPosition(Mensajes[i].NombreChat.getPosition().x, Mensajes[i].NombreChat.getPosition().y - aux);
